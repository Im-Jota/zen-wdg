  ...
  <div class="calendar-container" :class="theme === 'dark' ? 'dark' : 'light'">
    ...
    <div class="calendar-header">
      <div class="view-selector">
        ...
        >
          {{ view === 'day' ? i18n.viewDay : view === 'week' ? i18n.viewWeek : i18n.viewMonth }}
        </button>
      </div>
      <div class="navigation">
        ...
        <button @click="goToToday" class="today-btn">{{ i18n.today }}</button>
      </div>
      <div class="header-actions">
        <button @click="exportEventsAction" class="action-btn" :title="i18n.exportEvents">📤</button>
        <label class="action-btn" :title="i18n.importEvents">📥
          <input type="file" accept=".json" @change="importEventsAction" hidden>
        </label>
        <button @click="showEventModal = true" class="add-event-btn">{{ i18n.newEvent }}</button>
      </div>
    </div>
    ...
    <div class="calendar-view">
      ...
      <div v-show="currentView === 'day'" class="zcal-day-view">
        <div class="time-grid">
          <div v-for="hour in 24" :key="hour" class="hour-slot"
            ...
            @drop.prevent="onDropHour(formatDate(currentDate), hour - 1)"
          >
            ...
            <div class="hour-content">
              <div 
                ...
                :class="{ dragging: dragEventId === event.id }"
                draggable="true"
              ...
              >
                ...
                <div v-if="event.reminder" class="event-reminder">🔔</div>
        ...
        <div class="week-grid">
          <div v-for="hour in 24" :key="hour" class="week-hour-row">
            ...
            <div v-for="day in weekDays" :key="day.date" class="week-hour-cell"
              @dragover.prevent="onDragOver($event)"
              @drop.prevent="onDropHour(day.date, hour - 1)"
            >
              <div 
      ...
      <div v-show="currentView === 'month'" class="zcal-month-view">
        <div class="month-header">
          <div v-for="day in i18n.dayNames" :key="day" class="month-day-name">{{ day }}</div>
        </div>
    ...
    <div v-if="showEventModal" class="modal-overlay" @click.self="closeModal">
      <div class="modal-content">
        <div class="modal-header">
          <h3>{{ editingEvent ? i18n.editEvent : i18n.createEvent }}</h3>
          <button @click="closeModal" class="close-btn">✕</button>
        </div>
        <div class="modal-body">
          <div class="form-group">
            <label>{{ i18n.eventName }}</label>
            <input v-model="eventForm.name" type="text" :placeholder="i18n.eventNamePlaceholder">
          </div>
          <div class="form-group">
          ...
          <div class="form-group">
            <label>{{ i18n.reminder }}</label>
            <select v-model="eventForm.reminder">
              <option value="">{{ i18n.reminderNone }}</option>
              <option value="5">{{ i18n.reminder5 }}</option>
              <option value="15">{{ i18n.reminder15 }}</option>
              <option value="30">{{ i18n.reminder30 }}</option>
            </select>
          </div>
          <div class="form-group">
        ...
        <div class="modal-footer">
          <button v-if="editingEvent" @click="deleteEvent" class="delete-btn">{{ i18n.delete }}</button>
          <button @click="closeModal" class="cancel-btn">{{ i18n.cancel }}</button>
          <button @click="saveEvent" class="save-btn">{{ editingEvent ? i18n.save : i18n.create }}</button>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  computed: {
    i18n() {
      return getWidgetLocale('calendar', this.locale);
    },
    currentDateLabel() {
      const months = this.i18n.months;
    ...
    weekDays() {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        days.push({
          ...
          name: this.i18n.dayNames[date.getDay()],
          number: date.getDate()
        });
      }
      return days;
  ...
  methods: {
    ...
    async saveEvent() {
      if (!this.eventForm.name) {
        await window.ZenModals.showWarning({
          title: this.i18n.nameRequired,
          message: this.i18n.nameRequiredMsg
        });
        return;
      }
    ...
    async deleteEvent() {
      const confirmed = await window.ZenModals.showDeleteConfirm(this.i18n.deleteConfirm);
      if (confirmed) {
    ...
    onDropHour(dateString, hour) {
      if (!this.dragEventId) return;
      const event = this.events.find(e => e.id === this.dragEventId);
      if (!event) return;
      // Calculate duration
      const oldStart = parseInt(event.startTime.split(':')[0]);
      const oldEnd = parseInt(event.endTime.split(':')[0]);
      const duration = oldEnd - oldStart;
      // Update event
      event.date = dateString;
      event.startTime = `${String(hour).padStart(2, '0')}:00`;
      event.endTime = `${String(Math.min(hour + Math.max(duration, 1), 24)).padStart(2, '0')}:00`;
      this.dragEventId = null;
      this.saveEvents();
      this.scheduleReminders();
    },
    ...
    exportEventsAction() {
      if (this.events.length === 0) {
        if (window.ZenModals) {
          window.ZenModals.showWarning({ title: this.i18n.exportEvents, message: this.i18n.exportEmpty });
        }
        return;
      }
      ...
      reader.onload = (evt) => {
        try {
          ...
          if (Array.isArray(imported)) {
            ...
            imported.forEach(ev => { ev.id = Date.now() + Math.random(); });
            this.events.push(...imported);
            this.saveEvents();
            this.scheduleReminders();
            if (window.ZenModals) {
              window.ZenModals.showSuccess({ 
                title: this.i18n.importEvents, 
                message: this.i18n.importSuccess.replace('{count}', imported.length) 
              });
            }
          }
        } catch (err) {
          if (window.ZenModals) {
            window.ZenModals.showWarning({ title: this.i18n.importEvents, message: this.i18n.importError });
          }
        }
      };
    ...
    scheduleReminders() {
      ...
      this.events.forEach(event => {
        ...
        const minutesBefore = parseInt(event.reminder);
        if (!minutesBefore) return;
        
        const eventDateTime = new Date(`${event.date}T${event.startTime}`).getTime();
        const reminderTime = eventDateTime - minutesBefore * 60000;
    ...
    showReminder(event) {
      ...
      } else if (window.ZenModals) {
        window.ZenModals.showWarning({
          title: `🔔 ${this.i18n.reminder}`,
          message: `${event.name} — ${event.startTime}`
        });
      }
    },
...
.hour-slot.drag-over,
.week-hour-cell.drag-over {
  background: rgba(6, 109, 90, 0.12);
}
